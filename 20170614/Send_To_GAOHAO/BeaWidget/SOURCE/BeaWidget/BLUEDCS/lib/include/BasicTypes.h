...
struct _TIME_T 
{
    ...
    _TIME_T operator - (const _TIME_T &Right) const 
    {
		...
		//大数据，重载运算符/
	_TIME_T operator / (const double &Right) const 
    {
        _TIME_T Temp = *this;
...
struct _SEQUENCE_NUMBER_T 
{
    ...
    //重载运算符 -
    _SEQUENCE_NUMBER_T operator - (const unsigned long Sub) const
    {
        _SEQUENCE_NUMBER_T Temp = *this;
        if (Temp.ulLow < Sub)
        {
    ...
    _SEQUENCE_NUMBER_T operator - (const _SEQUENCE_NUMBER_T &Right) const 
    {
        ...
        {
            -- Temp.lHigh;
//            Temp.ulLow = Temp.ulLow + 4294967295 - Right.ulLow;
            Temp.ulLow = Temp.ulLow - Right.ulLow;
        }
        else
				Temp.ulLow -= Right.ulLow;
...
*/ 
#if defined VXWORKS || defined KYLIN  //移植
...
*/
#define DATAFRAG_LENGTH 30000
#define DATAMESSAGE_LIMIT 60000
...
#elif KYLIN
typedef int64_t _LONG_LONG;
typedef uint64_t _U_LONG_LONG;
#endif
