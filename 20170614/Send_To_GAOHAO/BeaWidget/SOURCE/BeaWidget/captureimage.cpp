...
void CaptureImage::setFile(char portName[100],double num,double size,QString path)
{
    ...
    qDebug()<<"路径数量保存成功, Path: "<<path <<endl;
}
...
int CaptureImage::ethdump_initSocket()
{
    ...
    /* 创建套接字 */
    fd = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL));//init
    if (fd < 0)
    {
        //Shang find the problem here.2017.02.28
...
void CaptureImage::ethdump_startCapture(const int fd)
{
 ...
 //   qDebug()<<"filePath " << filePath;
    ...
    QString fileName=filePath+QString::number(currentNum)+ "_"+ g_szIfName + "_" + startTime +".bin";
    //QByteArray temp1=("rm  -f " +filePath +"/" +QString::number(currentNum)+ "_"+g_szIfName+"_*.*").toLatin1();
    //QByteArray temp1=("rm  -f ./ImageFile/"+QString::number(currentNum)+ "_*.*").toLatin1();  //此序号文件如果存在，删除旧的，再写新的
    QByteArray temp1=("rm  -f  " + filePath + QString::number(currentNum)+ "_"+g_szIfName+"_*.*").toLatin1();  //此序号文件如果存在，删除旧的，再写新的     //yangmeng 161030 加入多网卡判断 //yangmeng 161101修改文件路径到1T硬盘
    ...
    //若文件打开成功，则记录当前序号到sysdict中,  2016.10.26 qhy update begin
        //query.prepare(" update sysdict set  value = ? where attr= '当前序号'");

       // query.prepare(" update sysdict set  value = ? where attr='当前序号"+strAdpID+"'");      //yangmeng 161101 加入多网卡序号判断
       //cur_seq = "当前序号"+strAdpID;
       ...
       while(whileFlag)
        {
            ...
            if( currentSize >= fileSize)
            {
                ...
                fileName=filePath+QString::number(currentNum)+ "_"+ g_szIfName + "_"+ startTime +".bin";
                //QByteArray temp=("rm  -f  ./ImageFile/"+QString::number(currentNum)+"_*.*").toLatin1();   //此序号文件如果存在，删除旧的，再写新的
                QByteArray temp=("rm  -f " + filePath +QString::number(currentNum)+ "_"+g_szIfName+"_*.*").toLatin1();  //此序号文件如果存在，删除旧的，再写新的     //yangmeng161101 增加双网卡判断 修改文件路径到1T硬盘
  ///run/media/root/3625-19EE/ImageFile/
                //query.prepare(" update sysdict set  value = ? where attr= '当前序号'");  //回填db中的当前序号值
                //query.prepare(" update sysdict set  value = ? where attr= '当前序号"+strAdpID+"'");      //yangmeng 161101 加入多网卡序号判断
                //cur_seq = '当前序号'+strAdpID;
                query.prepare(" update sysdict set  value = ? where attr= ? " );
                query.bindValue(0, QString::number(currentNum));
